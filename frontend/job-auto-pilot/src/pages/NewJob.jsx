...
import { Rocket, Loader2, Sparkles, Zap, GraduationCap, Check, Edit2, Globe, MapPin, BarChart2, Tag } from "lucide-react";
...
export default function NewJob() {
  ...
  const extractJobDetails = (description) => {
    ...
    // Extract key skills/keywords
    const skillPatterns = [
      /(?:required skills|qualifications|requirements):\s*([^\n]+(?:\n[•\-*]\s*[^\n]+)*)/i,
      /[•\-*]\s*([A-Z][a-zA-Z0-9+#.\s]{2,25})(?=\s*\n|$)/g
    ];
    const skills = new Set();
    const commonSkills = ['React', 'Python', 'JavaScript', 'AWS', 'Docker', 'SQL', 'Node.js', 'Java', 'C\\+\\+', 'TypeScript', 'Git'];
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="glass-card rounded-2xl p-12 max-w-md w-full mx-4">
              <div className="text-center">
                ...
                <h2 className="text-2xl font-bold text-white mb-3">Analyzing job description…</h2>
                <p className="text-white/40 mb-8">Extracting role, company, website, and key requirements</p>
        ...
        {showConfirm && extractedData && (
          <div>
            <div className="mb-8 text-center">
              <h1 className="text-4xl font-bold text-white mb-3">Confirm details</h1>
              <p className="text-lg text-white/40">Review extracted information before generating</p>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {editMode ? (
                      ...
                      <p className="text-lg font-semibold text-white">{extractedData.jobTitle}</p>
                    )}
                  </div>
                  {!editMode && (
                    <Button
                ...
                {extractedData.website && (
                  <div className="flex items-center gap-2">
                    ...
                    {editMode ? (
                      ...
                      <p className="text-sm text-white/60">{extractedData.website}</p>
                    )}
                  </div>
                )}
                ...
                {extractedData.location && (
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-white/60">{extractedData.location}</p>
                  </div>
                )}
                ...
                {extractedData.seniority && (
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-white/60">{extractedData.seniority}</p>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {extractedData.keywords.map((keyword, i) => (
                        <span key={i} className="px-3 py-1 rounded-full bg-purple-600/20 text-purple-400 text-xs">
                          {keyword}
                        </span>
                      ))}
                    </div>
        ...
        {!showConfirm && (
          <>
            <div className="mb-10 text-center">
              <h1 className="text-5xl font-bold mb-4" style={{ color: '#F5F5F7' }}>Create a new job packet</h1>
              <p className="text-xl" style={{ color: '#B3B3B8' }}>This will save you time</p>
            </div>

            <div className="glass-card rounded-2xl p-10 space-y-8 max-w-4xl mx-auto border border-white/10">
          {/* Resume Selector */}
          <div>
            <label className="block text-base font-semibold mb-3" style={{ color: '#F5F5F7' }}>
              Resume <span className="text-red-400">*</span>
            </label>
            {resumes.length > 0 ? (
              <Select value={selectedResume} onValueChange={setSelectedResume}>
                <SelectTrigger className="border-white/12 text-white py-6 text-base" style={{ background: '#141414' }}>
              ...
              <div className="p-6 rounded-xl border border-white/12 text-center" style={{ background: '#141414' }}>
                <p className="mb-3 text-sm" style={{ color: '#B3B3B8' }}>No resumes found</p>
                <Button
          ...
          {/* AI Mode Selector */}
          <div>
            <label className="block text-base font-semibold mb-2" style={{ color: '#F5F5F7' }}>
              AI Mode <span className="text-red-400">*</span>
            </label>
            <p className="text-sm mb-5" style={{ color: '#8A8A92' }}>Choose how AI handles your resume content.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="font-bold text-lg" style={{ color: '#F5F5F7' }}>Standard</span>
                </div>
                <span className="inline-block text-xs px-2.5 py-1 rounded-full bg-green-500/20 text-green-400 mb-4 font-semibold">Recommended • Safe & ATS-friendly</span>
                <ul className="text-sm leading-relaxed space-y-1.5" style={{ color: '#B3B3B8' }}>
                  ...
                  <li className="font-semibold text-green-400/80">• Does NOT create fake experience</li>
                </ul>
              </button>
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="font-bold text-lg" style={{ color: '#F5F5F7' }}>Elite</span>
                </div>
                <span className="inline-block text-xs px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-400 mb-4 font-semibold">Advanced • Use with caution</span>
                <ul className="text-sm leading-relaxed space-y-1.5 mb-3" style={{ color: '#B3B3B8' }}>
                  ...
                  <li className="font-semibold text-amber-400/80">• Higher risk if verified by employer</li>
                </ul>
                {aiMode === "elite" && (
          ...
          <div className="flex items-start gap-3 p-4 rounded-xl bg-white/5 border border-white/10">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-white">No experience / Student mode</span>
              </div>
...
- Strong problem-solving skills..."
            ...
            />
            <p className="text-xs mt-2" style={{ color: '#8A8A92' }}>Paste the full job description. We'll automatically extract role, company, and requirements.</p>
          </div>
